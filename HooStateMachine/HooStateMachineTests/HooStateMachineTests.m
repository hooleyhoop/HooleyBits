...
- (void)testTheSimplestStateMachineExample {
    ...
    NSMutableArray *allTransitions = [[[NSMutableArray alloc] init] autorelease];
    
	[allTransitions addObject:[idle_state addTransitionOn:doorClosed_event toState:active_state]];
    
    [idle_state addEntryAction:unlockDoorCmd];
    [idle_state addEntryAction:lockPanelCmd];
    
	[allTransitions addObject:[active_state addTransitionOn:drawerOpened_event toState:waitingForLight_state]];
	[allTransitions addObject:[active_state addTransitionOn:lightOn_event toState:waitingForDrawer_state]];

	[allTransitions addObject:[waitingForLight_state addTransitionOn:lightOn_event toState:unlockedPanel_state]];

	[allTransitions addObject:[waitingForDrawer_state addTransitionOn:drawerOpened_event toState:unlockedPanel_state]];

    [unlockedPanel_state addEntryAction:unlockPanelCmd];
    [unlockedPanel_state addEntryAction:lockDoorCmd];
	[allTransitions addObject:[unlockedPanel_state addTransitionOn:panelClosed_event toState:idle_state]];
	...
	// HooStateMachine_testCommandChannel testReciever = HooStateMachine_testCommandChannel alloc]();
     HooStateMachine *stateMachineInstance = [[[HooStateMachine alloc] initWithStartState:idle_state transitions:allTransitions resetEvents:nil] autorelease];
...
- (void)testTheSimplestStateMachineExampleWithJson {
    ...
    HooStateMachine *stateMachineInstance = [[[HooStateMachine alloc] initWithStartState:[simpleStateMachineParser state:@"st_idle"] transitions:[simpleStateMachineParser transitions] resetEvents:[simpleStateMachineParser resetEvents]] autorelease];
...
- (void)testAHierarchicalStateMachineExample {
	...
	HooStateMachineConfigurator *simpleStateMachineParser = [HooStateMachineConfigurator configNamed:@"hierarchical" inBundle:[NSBundle bundleForClass:[self class]]];   

    HooStateMachine *stateMachineInstance = [[[HooStateMachine alloc] initWithStartState:[simpleStateMachineParser state:@"st_off"] transitions:[simpleStateMachineParser transitions] resetEvents:[simpleStateMachineParser resetEvents]] autorelease];
