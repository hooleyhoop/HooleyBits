...
- (int)trace:(const char *)programname {
    ...
    if( _child_pid<0 ) {
        ...
        result = ptrace( PT_SIGEXC, _child_pid, 0, 0 );    // lldb // Get BSD signals as mach exceptions
        assert( result==0 );
        ...
        int result = task_for_pid( task_self, _child_pid, &_childTaskPort );
...
//                                 the bit on in it too).
        ...
        krc = task_set_exception_ports( _childTaskPort, EXC_MASK_ALL, _exceptionPort, EXCEPTION_DEFAULT | MACH_EXCEPTION_CODES, THREAD_STATE_NONE );
        if( krc != KERN_SUCCESS )
...
//    return;
    ...
    result = thread_get_state( _firstThread, MACHINE_THREAD_STATE, (thread_state_t)&thread_state, &stateCount );
    assert( result==0 );
    ...
    if( thread_state.tsh.flavor==x86_THREAD_STATE32 ){
       ...
       //         tState.__eflags &= ~mask_table[ 8 ];
            ...
            if( thread_set_state( _firstThread, MACHINE_THREAD_STATE, (thread_state_t)&thread_state, stateCount ) ) error((char *)"setting state");            
        }
        assert( bit8==1 );
