...
// returned values are released, we need to release anything else that we create
static AS3_Val encodeWavData( void *self, AS3_Val args ) {
    ...
    AS3_Val logFile_arg = AS3_Undefined();
	...
	// char buffer2[] = {'A','l','c','h','e'};
	// fwrite( buffer2, 1, sizeof(buffer2), _logFile );
	// get file size
	// fseek (_logFile, 0, SEEK_END);
	// long fileSize = ftell(_logFile);
	
	// char *stringToTest = "crrrraaannng";    
	// hooFileLog( "OUT> %s", stringToTest );
...
// returned values are released, we need to release anything else that we create
static AS3_Val encodeWavData_debug( void *self, AS3_Val args ) {
	...
	// 3) Debug Info Out
    AS3_Val lengthOfByteArray3 = AS3_GetS( logFile_arg, "length" );
